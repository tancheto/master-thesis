...
def generate_samples(no_of_samples,df,df_name):
                ...
                for i, each in enumerate(parent_candidate[key]):
                    temp_lst.append(parent_candidate[key][i] if cr < random.random() else
                                    int(parent_candidate[key][i] +
                                        f * np.subtract(child_candidate_1[key][i], child_candidate_2[key][i], dtype=np.float32)))
                new_candidate.append(temp_lst)
            else:
                new_candidate.append(abs(parent_candidate[key] + f * 
                                    np.subtract(child_candidate_1[key], child_candidate_2[key], dtype=np.float32)))    
    ...
    if df_name == 'Diabetes':
        final_df = final_df.rename(columns={								
            0:'race', 1:'gender', 2:'age', 3:'time_in_hospital', 4:'num_lab_procedures', 5:'num_procedures', 
            6:'num_medications', 7:'number_outpatient', 8:'number_emergency', 9:'number_inpatient', 10:'number_diagnoses', 
            11:'readmit_30_days'}, errors="raise")
